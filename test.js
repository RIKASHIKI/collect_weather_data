...
async function getCuacaToExcel() {
  try {
    ...
    dates.forEach(d => {
      ...
      targetHours.forEach(h => {
        const found = cuacaList.find(item => {
          const dt = new Date(item.local_datetime || item.datetime);
          return (
            dt.getHours() === h && dt.toISOString().slice(0, 10) === d
          );
        });
        ...
        if (found) {
          ...
          filteredList.push({
            local_datetime: dummyDate.toISOString(),
            datetime: dummyDate.toISOString(),
            t: null,
            hu: null,
    ...
    if (fs.existsSync(filename)) {
      ...
      worksheet = workbook.Sheets["test"];
      if (worksheet) {
      ...
      if (idx !== -1) {
        ...
        if (isDifferent) {
          ...
          if ((!oldHasData && newHasData) || (oldHasData && newHasData)) {
            ...
            console.log(`[UPDATE] ${datetimeFormatted}`);
            console.log(`   Dari: ${oldRow.join(" | ")}`);
            console.log(`   Ke  : ${newRow.join(" | ")}\n`);
            updated = true;
          } else {
            console.log(`[SKIP] ${datetimeFormatted} (data sudah ada isi)`);
          }
        } else {
